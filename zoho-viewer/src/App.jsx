...
// /* ── OpenRouter config ── */
// const OPENROUTER_API_KEY = "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";
...
// /* ══════════════════════════════════════════════
//    OPENROUTER SENTIMENT CLASSIFIER
// ══════════════════════════════════════════════ */
// async function classifyBatch(answers, categories, attempt = 0) {
//   if (!answers.length) return {};
//   const catList  = categories.join(" | ");
//   // Use index numbers for categories to minimise output tokens
//   const catMap   = Object.fromEntries(categories.map((c, i) => [i, c]));
//   const catIndex = categories.map((c, i) => `${i}=${c}`).join(", ");
//   const numbered = answers.map((a, i) => `${i + 1}. ${a}`).join("\n");

//   const prompt =
//     `Classify each answer. Categories: ${catIndex}.\n` +
//     `Reply ONLY with a JSON array of category numbers (0-${categories.length - 1}), one per answer.\n` +
//     `Example for 3 answers: [0,2,1]\n\n` +
...
//   if (res.status === 429) {
//     if (attempt >= 3) throw new Error("Rate limited after 3 retries — try again in a moment.");
//     await sleep(3000 * Math.pow(2, attempt));
//     return classifyBatch(answers, categories, attempt + 1);
//   }
...
//   let labels  = [];
//   try { labels = match ? JSON.parse(match[0]) : []; } catch (_) { labels = []; }

//   const counts = {};
//   categories.forEach((c) => (counts[c] = 0));
//   console.log("[classifyBatch] parsed labels:", labels);

//   labels.forEach((lbl) => {
//     // Try numeric index first (new compact format)
//     const idx = parseInt(lbl, 10);
//     if (!isNaN(idx) && catMap[idx]) {
//       counts[catMap[idx]] = (counts[catMap[idx]] || 0) + 1;
//       return;
//     }
//     // Fall back to fuzzy string match
//     const normalised = (String(lbl) || "").toLowerCase().replace(/[^a-z ]/g, "").trim();
//     const canonical = categories.find((c) => {
//       const cn = c.toLowerCase().replace(/[^a-z ]/g, "").trim();
//       return cn === normalised || normalised.includes(cn) || cn.includes(normalised);
//     });
//     if (canonical) counts[canonical] = (counts[canonical] || 0) + 1;
//     else console.warn("[classifyBatch] unmatched label:", lbl, "vs categories:", categories);
//   });
//   return counts;
// }
...
//             <span style={{ fontSize: 10, color: "#9CA3AF", width: 18, textAlign: "right", flexShrink: 0 }}>{count}</span>
//           </div>
//         );
//       })}
//     </div>
...
//   badgeAI:  { fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 5, background: "#EFF6FF", color: "#2563EB", whiteSpace: "nowrap" },
...
//     // Debug: log what answers are being picked up for q3
//     console.log("[Sentiment] q4 sample answers:", mapped.slice(0, 3).map(r => r.q4));
...
//     { key: "phone", label: "Phone",          find: (r) => r.phone },
//     { key: "ins",   label: "Insurance",      find: (r) => r.ins   },
//     { key: "q1",    label: "NPS",            find: (r) => r.q1    },
//     { key: "q2",    label: "Improve",        find: (r) => r.q2    },
//     { key: "q3",    label: "Policy Clarity", find: (r) => r.q3    },
//     { key: "q4",    label: "Resolution",     find: (r) => r.q4    },
//     { key: "q5",    label: "Fairness",       find: (r) => r.q5    },
//     { key: "ts",    label: "Time",           find: (r) => r.ts    },
//   ];
...
//       {/* NAV */}
//       <nav style={{ background: "#fff", borderBottom: "1px solid #E4E6EA", height: 58, display: "flex", alignItems: "center", padding: "0 28px", gap: 20, position: "sticky", top: 0, zIndex: 50, boxSizing: "border-box", width: "100%" }}>
//         <div style={{ display: "flex", alignItems: "center", gap: 9, marginRight: 12, flexShrink: 0 }}>
//           <div style={{ width: 32, height: 32, background: "#C8102E", borderRadius: 7, display: "grid", placeItems: "center", flexShrink: 0 }}>
...
//           </div>
//           <span style={{ fontSize: "13.5px", fontWeight: 700, whiteSpace: "nowrap" }}>Chola MS General Insurance</span>
//         </div>
//         <div style={{ width: 1, height: 20, background: "#E4E6EA", flexShrink: 0 }} />
//         <button style={{ padding: "6px 14px", borderRadius: 6, fontSize: 13, fontWeight: 600, color: "#fff", background: "#C8102E", border: "none", cursor: "pointer", fontFamily: "Inter,sans-serif", flexShrink: 0 }}>
//           Survey Dashboard
//         </button>
//         <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
//           <span style={{ fontSize: 12, color: "#9CA3AF", whiteSpace: "nowrap" }}>{now.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
//           <button
//             onClick={() => fetchData(true)}
//             disabled={refreshing}
//             style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: refreshing ? "not-allowed" : "pointer", border: "1px solid #E4E6EA", background: "#fff", color: refreshing ? "#9CA3AF" : "#374151", fontFamily: "Inter,sans-serif", transition: "all .2s", whiteSpace: "nowrap" }}
//           >
...
//             <div style={S.card}>
//               <div style={S.cardHd}>
//                 <h3 style={{ fontSize: "13.5px", fontWeight: 600, margin: 0 }}>NPS Breakdown</h3>
...
//                       <span style={{ flex: 1, fontSize: "12.5px", color: "#374151", minWidth: 0 }}>{row.label} <span style={{ color: "#9CA3AF", fontSize: 11 }}>{row.range}</span></span>
...
//         {/* 3. SATISFACTION BY QUESTION — AI Sentiment */}
//         <div style={{ marginBottom: 24 }}>
//           <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
//             <span style={{ ...S.secHd, marginBottom: 0 }}>Satisfaction by Question</span>
//             <span style={S.badgeAI}>✦ AI Sentiment Analysis</span>
//           </div>
...
//           <div style={S.card}>
//             <div style={S.cardHd}>
//               <h3 style={{ fontSize: "13.5px", fontWeight: 600, margin: 0 }}>Voice Survey Responses</h3>
...
//         {/* Centre-left — Survey Dashboard + Refresh */}
...
//           {/* Haiva logo + name */}
//           <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
//             {/* Haiva logo mark — stylised "H" wave icon */}
//             <div style={{ width: 30, height: 30, borderRadius: 8, background: "linear-gradient(135deg, #6366F1 0%, #8B5CF6 100%)", display: "grid", placeItems: "center", flexShrink: 0 }}>
//               <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
//                 <path d="M4 6 Q8 3 12 6 Q16 9 20 6" stroke="white" strokeWidth="2.2" strokeLinecap="round" fill="none"/>
//                 <path d="M4 12 Q8 9 12 12 Q16 15 20 12" stroke="white" strokeWidth="2.2" strokeLinecap="round" fill="none"/>
//                 <path d="M4 18 Q8 15 12 18 Q16 21 20 18" stroke="white" strokeWidth="2.2" strokeLinecap="round" fill="none"/>
//               </svg>
//             </div>
//             <div style={{ lineHeight: 1.15 }}>
//               <div style={{ fontSize: "13px", fontWeight: 700, color: "#111827", letterSpacing: "0.3px" }}>Haiva</div>
//               <div style={{ fontSize: "10px", color: "#9CA3AF", fontWeight: 500 }}>Voice AI Platform</div>
//             </div>
//           </div>
//         </div>
//       </nav>
...
//     type: "doughnut",
//     data: {
//       labels: ["Critical (0–6)", "Neutral (7–8)", "Happy (9–10)"],
//       datasets: [{ data: [det, pass, prom], backgroundColor: ["#C8102E", "#B45309", "#15803D"], borderWidth: 3, borderColor: "#fff", hoverOffset: 4 }],
//     },
//     options: { responsive: true, maintainAspectRatio: false, cutout: "72%", plugins: { legend: { display: false } } },
//   });
...
//     { key: "phone", label: "Phone",          find: (r) => r.phone },
//     { key: "ins",   label: "Insurance",      find: (r) => r.ins   },
//     { key: "q1", label: <><span>Recommendation</span><br/><span>Score</span></>, find: (r) => r.q1 },
//     { key: "q2",    label: "Improve",        find: (r) => r.q2    },
//     { key: "q3",    label: "Policy Clarity", find: (r) => r.q3    },
//     { key: "q4",    label: "Resolution",     find: (r) => r.q4    },
//     { key: "q5",    label: "Fairness",       find: (r) => r.q5    },
...
//           <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
//             <img
//               src="zoho-viewer\src\assets\haiva.png"
//               alt="Haiva"
//               style={{ width: 30, height: 30, objectFit: "contain", borderRadius: 6 }}
//             />
//             <div style={{ lineHeight: 1.15 }}>
//               <div style={{ fontSize: "13px", fontWeight: 700, color: "#111827", letterSpacing: "0.3px" }}>Haiva</div>
//               {/* <div style={{ fontSize: "10px", color: "#9CA3AF", fontWeight: 500 }}>AI Platform</div> */}
//             </div>
//           </div>
//         </div>
//         </div>
...
//           <h1 style={{ fontSize: 17, fontWeight: 700, margin: 0 }}>Customer Experience Dashboard</h1>
//           <p style={{ fontSize: "12.5px", color: "#9CA3AF", marginTop: 3, marginBottom: 0 }}>
//             Call Survey Analytics &nbsp;·&nbsp; {rows.length} responses collected &nbsp;·&nbsp; Last updated {now.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
//           </p>
//         </div>
...
//                 <span style={S.badgeRed}>Recommendation Score: {npsScore >= 0 ? "+" : ""}{npsScore}</span>
//               </div>
//               <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
//                 <div style={{ position: "relative", width: 150, height: 150, flexShrink: 0 }}>
//                   <NpsDonut prom={prom} pass={pass} det={det} />
//                   <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
//                     <div style={{ fontSize: 30, fontWeight: 700 }}>{npsScore >= 0 ? "+" : ""}{npsScore}</div>
//                     <div style={{ fontSize: 10, color: "#9CA3AF", fontWeight: 500 }}></div>
//                   </div>
//                 </div>
//                 <div style={{ flex: 1, minWidth: 0 }}>
//                   {[{ color: "#15803D", label: "Happy Customers",   desc: "Would recommend us",    range: "(9–10)", n: prom },
//                     { color: "#B45309", label: "Neutral Customers",  desc: "Satisfied but passive",  range: "(7–8)",  n: pass },
//                     { color: "#C8102E", label: "At-Risk Customers",  desc: "Had a poor experience",  range: "(0–6)",  n: det  }].map((row) => (
//                     <div key={row.label} style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 0", borderBottom: "1px solid #F0F1F3" }}>
//                       <span style={{ width: 8, height: 8, borderRadius: "50%", background: row.color, flexShrink: 0 }} />
//                       <div style={{ flex: 1, minWidth: 0 }}>
...
//             <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
//               <span style={{ fontSize: 12, color: "#9CA3AF", fontWeight: 500 }}>Filter:</span>
//               {[{ key: "all", label: "All" }, { key: "prom", label: "Happy (9–10)" }, { key: "det", label: "At-Risk (0–6)" },
//                 ...insTypes.map((ins) => ({ key: ins, label: ins.replace(" Insurance", "") }))
//               ].map(({ key, label }) => (
//                 <button key={key} onClick={() => setFilter(key)} style={{ padding: "4px 11px", borderRadius: 5, fontSize: 12, fontWeight: filter === key ? 600 : 500, cursor: "pointer", border: "1px solid", borderColor: filter === key ? "#C8102E" : "#E4E6EA", background: filter === key ? "#C8102E" : "#fff", color: filter === key ? "#fff" : "#6B7280", fontFamily: "Inter,sans-serif" }}>{label}</button>
//               ))}
...
//   return (
//     <span
//       style={{ position: "relative", display: "inline-flex", alignItems: "center", marginLeft: 4, cursor: "default", flexShrink: 0 }}
...
//           boxShadow: "0 4px 14px rgba(0,0,0,0.2)", zIndex: 9999, pointerEvents: "none",
